...
xtrabackup --backup --safe-slave-backup --slave-info ${lock_per_table_ddl} --stream=xbstream \
  --host=${DP_DB_HOST} --user=root --password=${MYSQL_ROOT_PASSWORD} --datadir=${DATA_DIR} \
...
# record server uuid
cat ${MYSQL_DIR}/auto.cnf | grep server-uuid | awk -F '=' '{print $2}' | datasafed push - "${DP_BACKUP_NAME}.server-uuid"
