...
# Do a temporary startup of the GreatDB Server, for init purposes
...
docker_temp_server_stop() {
	if ! greatdbadmin --defaults-extra-file=<( _mysql_passfile ) shutdown -uroot --socket="${SOCKET}"; then
...
docker_create_db_directories() {
	...
	if [ "$user" = "0" ]; then
		...
		find "$DATADIR" \! -user greatdb -exec chown greatdb '{}' +
	fi
}
...
docker_init_database_dir() {
	...
	if [ "$MYSQL_MAJOR" = '5.6' ]; then
		greatdb_install_db --datadir="$DATADIR" --rpm --keep-my-cnf "${@:2}"
	else
		"$@" --initialize-insecure
	fi
	mysql_note "Database files initialized"

	if command -v greatdb_ssl_rsa_setup > /dev/null && [ ! -e "$DATADIR/server-key.pem" ]; then
		# https://github.com/mysql/mysql-server/blob/23032807537d8dd8ee4ec1c4d40f0633cd4e12f9/packaging/deb-in/extra/mysql-systemd-start#L81-L84
		mysql_note "Initializing certificates"
		greatdb_ssl_rsa_setup --datadir="$DATADIR"
...
docker_process_sql() {
	...
	greatdb --defaults-extra-file=<( _mysql_passfile "${passfileArgs[@]}") --protocol=socket -uroot -hlocalhost --socket="${SOCKET}" --comments "$@"
}
...
docker_setup_db() {
	...
	if [ -z "$MYSQL_INITDB_SKIP_TZINFO" ]; then
		...
		greatdb_tzinfo_to_sql /usr/share/zoneinfo \
...
_main() {
	...
	if [ "$1" = 'greatdbd' ] && ! _mysql_want_help "$@"; then
		mysql_note "Entrypoint script for GreatDB Server ${MYSQL_VERSION} started."
		...
		if [ "$(id -u)" = "0" ]; then
			mysql_note "Switching to dedicated user 'greatdb'"
			exec gosu greatdb "$BASH_SOURCE" "$@"
		fi
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			echo
			mysql_note "GreatDB init process done. Ready for start up."
			echo
		fi
	fi
	if [ "$1" = 'greatdbd' ]; then
